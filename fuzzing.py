...
class Fuzzing:
    ...
    def gen_valid_query(self, query: list, table: gen.Table, tables: list, mut: bool, active: bool) -> tuple[int, list[str], gen.SQLNode]:
        '''
        ...
        for _ in range(self.max):
            ...
            if "Error" in msg and "constraint" not in msg and ERROR:
    ...
    def generate(self, cov: float, c, init_query: list, tables: list, nodes: list, 
                 find_best: bool = False, desc: str = "", mut: bool = False):
        '''
        ...
        while tries < self.threshold:
            ...
            if tables:
                ...
                while ((self.mod_table and (isinstance(table, gen.View) or (isinstance(table, gen.VirtualTable) and table.vtype == "dbstat"))) or 
                    ...
                    table = random.choice(updated_tables)
                cov_valid, valid_query, node, active = self.gen_valid_query(init_query, table, updated_tables, mut, active)
                valid_query = valid_query + random.choices([[gen.Optimization.random(table).sql() + ";"], []], weights=[0.05, 0.95], k=1)[0]
            else:
                cov_valid, valid_query, node, active = self.gen_valid_query(init_query, None, updated_tables, mut, active)
        ...
        pbar.close()
        return best_cov, best_c, new_query, updated_tables, self.corpus, best_msg
...
def run_pipeline(init_cov: int, init_query: list, init_tables: list, init_nodes: list, fuzz_pipeline: list[Fuzzing], 
    ...
    for i in range(repeat):
        for stage in fuzz_pipeline:
            cov, c, query, tables, nodes, msg = stage.generate(cov, c, query, tables, nodes, desc=desc)
            old_threshold = stage.threshold
            stage.threshold = mut_threshold
            cov, c, query, tables, nodes, msg = stage.generate(cov, c, query, tables, nodes, desc=desc, mut=True)
        ...
        if save:
            with open(TEST_FOLDER + f"{desc}_save_{cov}.txt", "w") as f:
                f.write(f"Best Coverage: {cov}, {c}, Valid/Invalid{total_valid}/{total_invalid}\n")
                f.write(f"Best Query: {query}\n")
                f.write(f"Tables: {tables}\n")
            with open(TEST_FOLDER + f"{desc}_error_{cov}.txt", "w") as f:
                for err in get_error(msg):
                    f.write(f"Message: {err}\n") # counts number of errors
            with open(TEST_FOLDER + f"{desc}_query_{cov}.sql", "w") as f:
                f.write("\n".join(query))
